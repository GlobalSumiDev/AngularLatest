...
.emp-type {
 ...
 {/*const handleShowFolders = () => {
  ...
  const deleteFolder = async (folder) => {
    if (!window.confirm(`Are you sure you want to delete folder '${folder.parentFolderName}'?`)) return;
    try {
      const token = localStorage.getItem('authToken');
      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const apiUrl = `https://app.globalsumi.com/api/folders?ParentFolderName=${encodeURIComponent(folder.parentFolderName)}&userEmail=${encodeURIComponent(userEmail)}`;
      const response = await fetch(apiUrl, { method: 'DELETE', headers });
      if (response.ok) {
        setFolders(folders.filter(f => f.parentFolderName !== folder.parentFolderName));
        alert(`Folder '${folder.parentFolderName}' deleted successfully!`);
        loadFolders(userEmail);
      }
    } catch (error) {
  ...
  const selectFolder = (folder) => {
    window.location.href = `/${folder.parentFolderName}/files`;
  };
          ...
          { /* <form onSubmit={createFolder}>
           ...
           { /*{employeesVisible && (
              <div style={{ marginBottom: '2rem' }}>
                <h3 className="employee-section-title">Employee List</h3>
                {employeesLoading ? (
